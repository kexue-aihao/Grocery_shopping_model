...
class GroceryModel {
  ...
  static double calculateDishBmiValue(Dish d, Profile p, String meal) {
    final double bmi = p.bmi == 0 ? 22.0 : p.bmi;
    final double target = getMealEnergyTarget(bmi, meal);
    final double energyScore = (34 - (d.kcal - target).abs() / target * 34).clamp(0, 34).toDouble();
    final double proteinScore = (d.protein / (meal == 'breakfast' ? 24 : 32) * 24).clamp(0, 24).toDouble();
    final double fiberScore = (d.fiber / (meal == 'breakfast' ? 6 : 8) * 18).clamp(0, 18).toDouble();
    final double fatPenalty = ((d.fat - (meal == 'breakfast' ? 14 : 18)) * .7).clamp(0, 12).toDouble();
    final double typeBonus = ['vegetable', 'soup', 'lean-protein', 'breakfast'].contains(d.type) ? 8.0 : 3.0;
    final double costScore = (16 - (d.cost - (meal == 'breakfast' ? 9 : 14)).abs() * .45).clamp(4, 16).toDouble();
    final double bmiAdjustment = bmi >= 24 && d.kcal < target ? 5.0 : bmi < 18.5 && d.protein >= 18 ? 5.0 : 0.0;
    return round((energyScore + proteinScore + fiberScore + typeBonus + costScore + bmiAdjustment - fatPenalty).clamp(0, 100).toDouble(), 0);
  }
