...
class GroceryModel {
  ...
  MarketPrice priceInfo(String ing)=>marketPrices.firstWhere((p)=>p.item==ing||ing.contains(p.item)||p.item.contains(ing), orElse:()=>MarketPrice(item:ing,category:'未分类',nationalAvg:10,regions:{'national':10})); double unitPrice(String ing,String region){final p=priceInfo(ing);return p.regions[region]??p.regions['national']??p.nationalAvg;} double qty(String ing,Dish d){if(RegExp('酱|醋|椒|葱|姜|蒜|糖|油|料|茶|淀|可').hasMatch(ing))return .012;if(RegExp('米|面|粉|饭|馍|麦|粥|包|油条').hasMatch(ing))return d.type=='breakfast'?.11:.09;if(RegExp('牛|羊|猪|肉|排骨|鸡|鱼|虾|蛤|海蛎').hasMatch(ing))return d.type=='soup'?.12:.14;if(RegExp('豆腐|腐竹|豆花|黄豆|牛奶|鸡蛋|皮蛋').hasMatch(ing))return .12;if(RegExp('蘑|菇|木耳|笋|海带').hasMatch(ing))return .045;return d.type=='vegetable'?.18:.10;} double estimateDishCost(Dish d,String region)=>round(math.max(1.5,d.ingredients.fold(0.0,(s,i)=>s+unitPrice(i,region)*qty(i,d))),1);
